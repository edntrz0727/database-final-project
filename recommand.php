...
<h3 align="center" style="color:brown">館員推薦</h3>
    <div align="center">
		<form action="searchrecommand.php" method="post">
			<input class="search-bar" type="text" name="search" placeholder="輸入尋找ISBN">
	...
	<table style="width:50%" align="center">
		...
		<?php   
                ...
                $search=$_POST["search"];
				$sql = "SELECT * FROM mouthlist limit $count, 10";
    ...
    <div id="move" class="move">
                <?php
                    if($page > 1){
                        echo "<form action='recommand.php?page=".($page-2)."&count=".($count-20)."' method='post'>";
                        echo "<button><</button>&ensp;";
                        echo "</form>";
                    }
                ?>
                <?php echo $page ?>&ensp;
                <form action="recommand.php?page=<?php echo $page ?>&count=<?php echo $count ?>" method='post'>
                    <button>></button>
                </form>
        </div>
