...
<h3 align="center" style="color:brown">搜尋結果</h3>
	...
	<table style="width:50%" align="center">
		...
		<?php   
                ...
                if($search == ''){
                    $countnull = 1;
                    header("location:j_borrow.php");
                }else{
                    $sql = "SELECT * FROM j_borrow where libraryID like '%$search%' limit $count, 10 ";
                }
    ...
    <div id="move" class="move">
                <?php
                    if($page > 1){
                        echo "<form action='searchj_borrow.php?page=".($page-2)."&count=".($count-20)."' method='post'>";
                        echo "<button><</button>&ensp;";
                        echo "</form>";
                    }
                    echo "$page&ensp;";
                    if($result->num_rows == 0){
                        echo "<form action='searchj_borrow.php?page=".$page."&count=".$count."' method='post'>";
                        echo "<button>></button>";
                        echo "</form>";
                    }
                ?>
