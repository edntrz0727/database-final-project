...
<body>
	...
	<table style="width:50%" align="center">
		...
		<?php   
                ...
                $search=$_POST["search"];
				$sql = "SELECT * FROM b_borrow where libraryID = '$libraryID' limit $count, 10";
    ...
    <div id="move" class="move">
                <?php
                    if($page > 1){
                        echo "<form action='allb_borrow.php?page=".($page-2)."&count=".($count-20)."&libraryID=".$libraryID."' method='post'>";
                        echo "<button><</button>&ensp;";
                        echo "</form>";
                    }
                ?>
                <?php echo $page ?>&ensp;
                <form action="allb_borrow.php?page=<?php echo $page ?>&count=<?php echo $count ?>&libraryID=<?php echo $libraryID ?>" method='post'>
                    <button>></button>
                </form>
        </div>
